...
export default function UpdateCake() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/cakes/updatecake/${formData._id}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
    ...
    <div className="p-3 max-w-3xl mx-auto min-h-screen">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className='flex flex-col gap-4 sm:flex-row justify-between'>
         ...
         <Select onChange={(e) => setFormData({ ...formData, type: e.target.value })} value={formData.type || 'uncategorized'}>
            <option value='uncategorized'>Uncategoried</option>
